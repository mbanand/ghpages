...
ui <- webawesomePage(
  ...
  tags$style(HTML("
    ...
    .sidebar-callout-text {
      margin: 0;
      line-height: 1.5;
      color: color-mix(in srgb, var(--wa-color-text-normal, #1f2937) 78%, white);
    }
    ...
    wa_container(
      ...
      wa_card(
        ...
        wa_container(
          ...
          wa_container(
            class = "badge-row",
            wa_badge("Iris", appearance = "filled"),
            wa_badge("Base graphics", appearance = "filled"),
            wa_badge("Reactive tabs", appearance = "filled")
          ),
          h2(class = "title-line", "Polished analytics with Shiny + Web Awesome"),
          p(
            class = "title-copy",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$preset_mode, {
  ...
  output$tab_specific_controls <- renderUI({
    ...
    if (active_tab == "chart") {
      wa_container(
        ...
        tags$script(HTML("
          setTimeout(() => {
            const y = document.getElementById('y_var');
            const s = document.getElementById('show_smoother');
  ...
  output$iris_plot <- renderPlot({
    ...
    if (isTRUE(input$show_smoother %||% TRUE) && nrow(data) > 1) {
  ...
  output$inspector_state <- renderUI({
    ...
    wa_container(
      ...
      wa_badge(paste("X:", input$x_var %||% "Sepal.Length"), appearance = "filled"),
      wa_badge(paste("Y:", input$y_var %||% "Petal.Width"), appearance = "filled")
    )
  })
}
